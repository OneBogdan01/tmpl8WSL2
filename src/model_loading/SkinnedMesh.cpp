...
void SkinnedMesh::LoadDiffuseTexture(const string& Dir, const aiMaterial* pMaterial, int index)
{
	...
	if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0)
	{
		...
		if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS)
		{
			...
			if (!m_Materials[index].pDiffuse->IsInit())
			{
...
void SkinnedMesh::LoadSpecularTexture(const string& Dir, const aiMaterial* pMaterial, int index)
{
	...
	if (pMaterial->GetTextureCount(aiTextureType_SHININESS) > 0)
	{
		...
		if (pMaterial->GetTexture(aiTextureType_SHININESS, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS)
		{
			...
			if (!m_Materials[index].pSpecularExponent->IsInit())
			{
...
void SkinnedMesh::PopulateBuffers()
{
	...
	glVertexAttribIPointer(BONE_ID_LOCATION, MAX_NUM_BONES_PER_VERTEX, GL_INT, sizeof(VertexBoneData),
		(const GLvoid*)0);
...
void SkinnedMesh::ReadNodeHierarchy(float AnimationTimeTicks, const aiNode* pNode, const Matrix4f& ParentTransform)
{
	...
	if (m_BoneNameToIndexMap.find(NodeName) != m_BoneNameToIndexMap.end())
	{
		...
		m_BoneInfo[BoneIndex].FinalTransformation = m_GlobalInverseTransform * GlobalTransformation * m_BoneInfo[
			BoneIndex].OffsetMatrix;
	}
